...
if (! -e $pcfgfile) {
	...
	$pcfg->param("MAIN.CONFIG_VERSION", "3");
	$pcfg->write($pcfgfile);
}
...
if ($pcfg->param("MAIN.CONFIG_VERSION") eq "2") {
	LOGDEB "Updating plugin config from version 2 to 3";
	print "Updating plugin config from version 2 to 3<br>";
	
	$pcfg->param("MAIN.CONFIG_VERSION", "3");
	# add new parameter values
	$pcfg->param("DEVICE.TYPE", "local");
	$pcfg->write($pcfgfile);
}
...
$BWT_TYPE = $pcfg->param("DEVICE.TYPE");
$BWT_IP = $pcfg->param("DEVICE.IP");
$BWT_CODE = $pcfg->param("DEVICE.CODE");
$BWT_PRODUCT_CODE = $pcfg->param("DEVICE.PRODUCT_CODE");
$BWT_API_KEY = $pcfg->param("DEVICE.API_KEY");
...
LOGDEB "BWT TYPE: ".$BWT_TYPE;
LOGDEB "BWT IP: ".$BWT_IP;
LOGDEB "BWT CODE: ".$BWT_CODE;
LOGDEB "BWT PRODUCT CODE: ".$BWT_PRODUCT_CODE;
LOGDEB "BWT API KEY: ".$BWT_API_KEY;
...
if ($TEST_MODE) {
	...
	print "- BWT TYPE: ".$BWT_TYPE."<br>";
	print "- BWT IP: ".$BWT_IP."<br>";
	print "- BWT CODE: ".$BWT_CODE."<br>";
	print "- BWT PRODUCT CODE: ".$BWT_PRODUCT_CODE."<br>";
	print "- BWT API KEY: ".$BWT_API_KEY."<br>";
	...
	if ($response->code == 200) {
		...
		if ($TEST_MODE) {
			...
			if ($response->code == 501) {
				printf "Probably your BWT Aqua firmware is to new, local access using this plugin is no longer supported by BWT.";
				printf "Check out the new BWT Aqua local REST API, see: https://wiki.loxberry.de/plugins/bwt_aqua/start";
			}
...
sub bwt_cloud
{
	if ($TRIGGER_MODE) {
		####################################################################################
		# (a) Trigger action in BWT Cloud
		####################################################################################
		$ID = $R::id;			# BWT Action ID (action name)
		$VALUE = $R::value; 	# BWT Action value (action parameter)
		
		if ($TEST_MODE) {
			print "Accessing BWT Cloud<br>";
		}
		
		my $url = 'https://bwt-real-smarthome-backend.azurewebsites.net/api/Perla/'.$BWT_PRODUCT_CODE.'/'.$ID.'/'.$VALUE;
		my $header = ['ApiKey' => $BWT_API_KEY];
		my $req = HTTP::Request->new('GET', $url, $header );
		my $ua = LWP::UserAgent->new();
		push @{ $ua->requests_redirectable }, 'PUT';

		print "Accessing BWT Cloud<br> url=".$url;
	...
	# (b) Read values
	####################################################################################
		my $url = 'https://bwt-real-smarthome-backend.azurewebsites.net/api/Perla/'.$BWT_PRODUCT_CODE;
		my $header = ['ApiKey' => $BWT_API_KEY];
		my $req = HTTP::Request->new('GET', $url, $header );
		my $ua = LWP::UserAgent->new();
		
		if ($TEST_MODE) {
			print "Login to BWT Cloud<br>";
		}
		...
		## actualizeData values
		print "displayName=".$decoded_json->{displayName}."<br>";
		print "stateTitle=".$decoded_json->{stateTitle}."<br>";
		print "stateBody=".$decoded_json->{stateBody}."<br>";
		print "deviceState=".$decoded_json->{deviceState}."<br>";
		print "lastTimeDataReceived=".$decoded_json->{lastTimeDataReceived}."<br>";
		print "remainingAmountOfResourcePercent=".$decoded_json->{remainingAmountOfResourcePercent}."<br>";
		print "refillResourceInDays=".$decoded_json->{refillResourceInDays}."<br>";
		print "isHolidayModeActive=".$decoded_json->{isHolidayModeActive}."<br>";
		print "scheduledHolidayModeStart=".$decoded_json->{scheduledHolidayModeStart}."<br>";
		print "firmwareVersion=".$decoded_json->{firmwareVersion}."<br>";
		print "waterTreatedTotal=".$decoded_json->{waterTreatedTotal}."<br>";
	...
	LOGDEB "Accessing BWT Cloud finished";
	return;	
}